...
interface EditorActions {
  openTab: (paneId: string, path: string, name: string, content: string, language: string) => void;
...
const generateTabId = (path: string) => `tab-${path.replace(/[^a-zA-Z0-9]/g, "-")}`;
...
export const useEditorStore = create<EditorState & EditorActions>((set, get) => ({
  ...
  openTab: (paneId, path, name, content, language) => {
    ...
    if (state.tabs.has(id)) {
      set((s) => ({
        panes: s.panes.map((p) =>
          p.id === paneId
            ? { ...p, activeTabId: id, tabIds: p.tabIds.includes(id) ? p.tabIds : [...p.tabIds, id] }
            : p
        ),
        activePaneId: paneId,
      }));
    ...
    const tab: EditorTab = { id, path, name, content, isDirty: false, language };
    ...
    set((s) => {
      ...
      return {
        tabs: newTabs,
        panes: s.panes.map((p) =>
          p.id === paneId ? { ...p, tabIds: [...p.tabIds, id], activeTabId: id } : p
        ),
        activePaneId: paneId,
      };
    });
  ...
  closeTab: (paneId, tabId) => {
    set((s) => {
      ...
      const newActiveTabId = pane.activeTabId === tabId ? newTabIds[newTabIds.length - 1] || "" : pane.activeTabId;

      // Check if tab is used in other panes
      const isUsedElsewhere = s.panes.some((p) => p.id !== paneId && p.tabIds.includes(tabId));
  ...
  closeSplit: () => {
    set((s) => {
      ...
      const mergedTabIds = [...new Set([...mainPane.tabIds, ...secondPane.tabIds])];
