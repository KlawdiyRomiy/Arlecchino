...
import { X, Check, Box, Layers, Database, Sprout, Factory, Mail, Bell, Shield, Zap, FileCode, PlayCircle, Package } from 'lucide-react';
...
const formConfigs: Record<string, {
  ...
  'make-model': {
    ...
    fields: [
      { name: 'name', label: 'Model Name', type: 'text', placeholder: 'Post', required: true },
  ...
  'make-controller': {
    ...
    fields: [
      { name: 'name', label: 'Controller Name', type: 'text', placeholder: 'PostController', required: true },
      { name: 'resource', label: 'Resource Controller', type: 'checkbox' },
      { name: 'api', label: 'API Controller', type: 'checkbox' },
      { name: 'invokable', label: 'Invokable (Single Action)', type: 'checkbox' },
      { name: 'model', label: 'Model Name (optional)', type: 'text', placeholder: 'Post' },
  ...
  'make-seeder': {
    ...
    fields: [
      { name: 'name', label: 'Seeder Name', type: 'text', placeholder: 'PostSeeder', required: true },
    ],
    handler: async (name, opts) => {
  ...
  'make-factory': {
    ...
    fields: [
      { name: 'name', label: 'Factory Name', type: 'text', placeholder: 'PostFactory', required: true },
  ...
  'make-mail': {
    ...
    fields: [
      { name: 'name', label: 'Mailable Name', type: 'text', placeholder: 'WelcomeMail', required: true },
      { name: 'markdown', label: 'Markdown Template', type: 'text', placeholder: 'emails.welcome' },
    ],
    handler: async (name, opts) => {
  ...
  'make-notification': {
    ...
    fields: [
      { name: 'name', label: 'Notification Name', type: 'text', placeholder: 'OrderShipped', required: true },
    ],
    handler: async (name, opts) => {
  ...
  'make-policy': {
    ...
    fields: [
      { name: 'name', label: 'Policy Name', type: 'text', placeholder: 'PostPolicy', required: true },
      { name: 'model', label: 'Model Name', type: 'text', placeholder: 'Post' },
    ],
    handler: async (name, opts) => {
  ...
  'make-job': {
    ...
    fields: [
      { name: 'name', label: 'Job Name', type: 'text', placeholder: 'ProcessPodcast', required: true },
  ...
  'make-event': {
    ...
    fields: [
      { name: 'name', label: 'Event Name', type: 'text', placeholder: 'OrderCreated', required: true },
    ],
    handler: async (name, opts) => {
  ...
  'make-resource': {
    ...
    fields: [
      { name: 'name', label: 'Resource Name', type: 'text', placeholder: 'PostResource', required: true },
  ...
  'make-migration': {
    ...
    fields: [
      { name: 'name', label: 'Migration Name', type: 'text', placeholder: 'create_posts_table', required: true },
      { name: 'create', label: 'Table to Create', type: 'text', placeholder: 'posts' },
      { name: 'table', label: 'Existing Table to Modify', type: 'text', placeholder: 'users' },
    ],
    handler: async (name, opts) => {
  ...
  'composer-require': {
    ...
    fields: [
      { name: 'name', label: 'Package Name', type: 'text', placeholder: 'spatie/laravel-permission', required: true },
        ...
        <form onSubmit={handleSubmit} className="flex flex-col max-h-[60vh]">
                ...
                {field.type === 'text' && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-laravel-red/50 focus:border-laravel-red"
                    />
                  </div>
                )}
                ...
                {field.type === 'checkbox' && (
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 text-laravel-red border-gray-300 rounded focus:ring-laravel-red"
                    />
                ...
                {field.type === 'select' && (
                  <div>
                    ...
                    <select
                      value={formData[field.name] || ''}
                      onChange={(e) => handleChange(field.name, e.target.value)}
                      className="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-laravel-red/50 focus:border-laravel-red"
                    >
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-black border-2 border-black/30 border-t-black bg-laravel-red hover:bg-laravel-red/90 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
