...
test("tui terminal overlay is viewport-positioned", async ({ page }) => {
  ...
  const [mainLayoutSource, panelRendererSource, floatingPanelSource] =
    await page.evaluate(async () => {
      const [layoutResponse, rendererResponse, panelResponse] =
        await Promise.all([
          fetch("/src/components/layout/MainLayout.tsx?raw"),
          fetch("/src/components/layout/MainLayoutPanelRenderer.tsx?raw"),
          fetch("/src/components/ui/FloatingPanel.tsx?raw"),
        ]);

      return [
        layoutResponse.ok ? await layoutResponse.text() : "",
        rendererResponse.ok ? await rendererResponse.text() : "",
        panelResponse.ok ? await panelResponse.text() : "",
      ];
    });

  expect(`${mainLayoutSource}\n${panelRendererSource}`).toMatch(
