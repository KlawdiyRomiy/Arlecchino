...
const blackprintEditor: EditorThemePalette = {
  ...
  scrollbarThumb: "#434343",
  scrollbarThumbHover: "#696969",
  border: "rgba(231, 231, 231, 0.105)",
  borderStrong: "rgba(231, 231, 231, 0.2)",
  text: "#dfdfdf",
  textSoft: "#aaaaaa",
  textMuted: "#858585",
  caret: "#ededed",
  activeLine: "rgba(231, 231, 231, 0.042)",
  activeLineGutter: "#d6d6d6",
  selection: "rgba(231, 231, 231, 0.15)",
  selectionInactive: "rgba(231, 231, 231, 0.09)",
  selectionMatch: "rgba(231, 231, 231, 0.075)",
  bracketMatch: "rgba(231, 231, 231, 0.105)",
  searchMatch: "rgba(231, 231, 231, 0.11)",
  tooltipBg: "#040404",
  tooltipBgStrong: "#0c0c0c",
  tooltipShadow:
    "inset 0 1px 0 rgba(255, 255, 255, 0.045), 0 18px 38px -24px rgba(0, 0, 0, 0.86), 0 28px 70px -42px rgba(0, 0, 0, 0.82)",
  ghostText: "rgba(223, 223, 223, 0.32)",
  highlight: "rgba(203, 203, 203, 0.105)",
  comment: "#7d7d7d",
  string: "#9d9d9d",
  number: "#a0a0a0",
  keyword: "#dbdbdb",
...
const blackprintTerminal: ITheme = {
  ...
  cursor: "#e8e8e8",
  cursorAccent: "#000000",
  selectionBackground: "rgba(232, 232, 232, 0.18)",
  black: "#000000",
  red: "#b3b3b3",
  green: "#d4d4d4",
  yellow: "#a3a3a3",
...
const arlecchinoMatineeEditor: EditorThemePalette = {
  background: "#FFF9E8",
  surface: "#F7E7BC",
  surfaceElevated: "#EED79B",
  gutter: "#F9EBC6",
  scrollbarTrack: "#EED79B",
  scrollbarThumb: "#D4B26A",
  scrollbarThumbHover: "#A66E1F",
...
const createThemeCssVariables = (
  ...
  const lightWorkspaceSurface = (surface: string, baseWeight: number) =>
    dark
      ? surface
      : `color-mix(in srgb, ${colors.bg} ${baseWeight}%, ${surface})`;
  const panelSurface = lightWorkspaceSurface(colors.bgPanel, 82);
  const editorBackground = lightWorkspaceSurface(editor.background, 88);
  const editorSurface = lightWorkspaceSurface(editor.surface, 80);
  const editorSurfaceElevated = lightWorkspaceSurface(
    editor.surfaceElevated,
    72,
  );
  const editorGutter = lightWorkspaceSurface(editor.gutter, 78);
  const editorScrollbarTrack = lightWorkspaceSurface(editor.scrollbarTrack, 70);
  const editorTooltipBackground = lightWorkspaceSurface(editor.tooltipBg, 84);
  const editorTooltipBackgroundStrong = lightWorkspaceSurface(
    editor.tooltipBgStrong,
    76,
  );
  const createStatusTone = (status: string) => ({
    text: `color-mix(in srgb, ${status} ${dark ? 82 : 20}%, ${colors.textPrimary})`,
    surface: `color-mix(in srgb, ${status} ${dark ? 16 : 24}%, ${colors.bgPanel})`,
    border: `color-mix(in srgb, ${status} ${dark ? 44 : 58}%, ${colors.border})`,
  });
  const successTone = createStatusTone(options.statuses.success);
  const warningTone = createStatusTone(options.statuses.warning);
  const errorTone = createStatusTone(options.statuses.error);
  const infoTone = createStatusTone(options.statuses.info);
  ...
  return {
    ...
    "--button-primary-bg": options.buttonPrimaryBg ?? colors.textPrimary,
    "--button-primary-bg-hover":
      options.buttonPrimaryBgHover ??
      `color-mix(in srgb, ${colors.textPrimary} 88%, ${colors.bgPanel})`,
    "--button-primary-text": options.buttonPrimaryText ?? colors.bg,
    "--button-primary-border":
      options.buttonPrimaryBorder ?? colors.textPrimary,
    "--button-primary-border-hover":
      options.buttonPrimaryBorderHover ??
      `color-mix(in srgb, ${colors.textPrimary} 88%, ${colors.bgPanel})`,
    ...
    "--status-success-text": successTone.text,
    "--status-success-surface": successTone.surface,
    "--status-success-border": successTone.border,
    "--status-warning-text": warningTone.text,
    "--status-warning-surface": warningTone.surface,
    "--status-warning-border": warningTone.border,
    "--status-error-text": errorTone.text,
    "--status-error-surface": errorTone.surface,
    "--status-error-border": errorTone.border,
    "--status-info-text": infoTone.text,
    "--status-info-surface": infoTone.surface,
    "--status-info-border": infoTone.border,
    "--explorer-file-label-0": `color-mix(in srgb, ${options.accentBrand} 82%, ${colors.textPrimary})`,
    "--explorer-file-label-1": `color-mix(in srgb, ${options.statuses.info} 82%, ${colors.textPrimary})`,
    "--explorer-file-label-2": `color-mix(in srgb, ${options.statuses.success} 82%, ${colors.textPrimary})`,
    "--explorer-file-label-3": `color-mix(in srgb, ${options.statuses.warning} 78%, ${colors.textPrimary})`,
    "--explorer-file-label-4": `color-mix(in srgb, ${options.statuses.error} 82%, ${colors.textPrimary})`,
    "--explorer-file-label-5": `color-mix(in srgb, ${editor.type} 82%, ${colors.textPrimary})`,
    "--explorer-file-label-6": `color-mix(in srgb, ${editor.property} 82%, ${colors.textPrimary})`,
    "--explorer-file-label-7": `color-mix(in srgb, ${editor.keyword} 80%, ${colors.textPrimary})`,
    "--explorer-file-label-8": `color-mix(in srgb, ${editor.function} 78%, ${colors.textPrimary})`,
    "--explorer-file-label-9": `color-mix(in srgb, ${colors.textSecondary} 88%, ${colors.textPrimary})`,
    ...
    "--editor-bg": editorBackground,
    "--editor-surface": editorSurface,
    "--editor-surface-elevated": editorSurfaceElevated,
    "--editor-gutter": editorGutter,
    "--editor-scrollbar-track": editorScrollbarTrack,
    ...
    "--editor-tooltip-bg": editorTooltipBackground,
    "--editor-tooltip-bg-strong": editorTooltipBackgroundStrong,
...
const createCustomThemeVariableOptions = (
  ...
  return {
    ...
    buttonPrimaryBg: accentBrand,
    buttonPrimaryBgHover: `color-mix(in srgb, ${accentBrand} 88%, ${dark ? colors.bg : colors.textPrimary})`,
    buttonPrimaryText: dark ? colors.bg : colors.bgPanel,
    buttonPrimaryBorder: `color-mix(in srgb, ${accentBrand} 88%, ${dark ? colors.bg : colors.textPrimary})`,
    buttonPrimaryBorderHover: `color-mix(in srgb, ${accentBrand} 76%, ${dark ? colors.bg : colors.textPrimary})`,
    accentBrand,
    accentBrandSoft: `color-mix(in srgb, ${accentBrand} 14%, ${colors.bgPanel})`,
    statuses: {
...
const blackprintTheme = createThemeDefinition({
  id: "blackprint",
  name: "Arlecchino Black",
  appearance: "dark",
  description: "Monochrome dark inversion of Arlecchino Light.",
  colors: {
    bg: "#000000",
    bgSecondary: "#080808",
    bgTertiary: "#111111",
...
const createCatppuccinTheme = (
  ...
  const editor: EditorThemePalette = {
    ...
    tooltipBg: dark ? flavor.mantle : flavor.base,
    tooltipBgStrong: dark ? flavor.surface0 : flavor.mantle,
    tooltipShadow: dark
  ...
  return createThemeDefinition({
    ...
    variableOptions: {
      ...
      accentBrandSoft: `color-mix(in srgb, ${flavor.blue} ${dark ? 16 : 12}%, ${colors.bgPanel})`,
      statuses: {
