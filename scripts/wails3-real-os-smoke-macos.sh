...
open_registered_target() {
  ...
  if [[ "$target" == *"://"* ]]; then
    ...
    if osascript -e "tell application id \"$BUNDLE_ID\" to open location \"$escaped_target\"" 2> "$TMP_ROOT/osascript-url.err"; then
      OS_OPEN_ROUTE="osascript open location"
      return 0
    fi
  else
    if osascript -e "tell application id \"$BUNDLE_ID\" to open POSIX file \"$escaped_target\"" 2> "$TMP_ROOT/osascript-file.err"; then
...
open_registered_target "$PROTOCOL_FILE_URL"
...
open_registered_target "$PROTOCOL_PREVIEW_URL"
...
open_registered_target "$PROTOCOL_FOCUS_URL"
...
open_registered_target "$PROTOCOL_REJECTED_URL"
...
open_registered_target "$MAIN_FILE"
...
' "$TRACE" "$REPORT" "$BUNDLE_ID" "$APP_BUNDLE_REAL" "$OS_OPEN_ROUTE"
