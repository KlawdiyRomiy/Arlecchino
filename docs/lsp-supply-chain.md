...
Status: alpha policy and release checklist.
...
For the alpha release, keep support for all currently wired language servers,
but treat the install channel differently:

- Homebrew, npm, Go, pip, composer, cargo, rustup, and gem installs are
  acceptable for source alpha because the user intentionally runs bootstrap or
  approves an in-app install and each ecosystem handles its own package
  resolution.
- The macOS alpha install path uses Homebrew for the previously direct binary
...
- Direct binary downloads are higher risk because Arlecchino is choosing and
  ...
  disabled or document manual installation for the public alpha.
...
- The default bootstrap may install the documented alpha toolchain after the
...
- Public alpha MCP approval should be a UI prompt unless the developer has
...
Unsigned macOS alpha bundles or DMGs are acceptable for early testers if the
