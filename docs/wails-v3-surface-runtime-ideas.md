...
- `frontend/src/wails/app.ts` переэкспортирует generated bindings из
  `frontend/src/bindings/arlecchino/app`.
...
1. Stabilize Wails v3 adapters: runtime wrapper, generated bindings policy, event
   normalization, `wails.json` migration notes.
2. Add Shell Capabilities Service, because every later feature needs capability checks.
3. Add Surface Runtime adapter over existing `PreviewWindow`/panel stores without changing
   visual behavior.
4. Add Event Contract Tests around current events before multi-window work.
5. Build Arlehub GUI hub mode as central surface using existing floating helpers.
6. Add Agent Flight Recorder and wire it to Arlehub timeline.
7. Add Applet Promotion Chain up to fullscreen/floating/snapped first.
8. Spike Wails v3 detached windows with Window Lease System.
9. Add native context menus and window roles for the most used surfaces.
10. Add Protocol Router, file associations and single instance.
11. Add Background Job Broker, tray and notifications.
12. Delay auto-updates/material/backdrop/dock badges until shell behavior is stable.
