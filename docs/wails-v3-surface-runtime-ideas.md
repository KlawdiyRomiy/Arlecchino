...
запускает Wails v3 alpha генератор pinned к `v3.0.0-alpha.87`. Dry-run по умолчанию пишет
...
- Добавлен live native delivery smoke harness:
  `./scripts/wails3-native-delivery-live-smoke-macos.sh` запускает production-shaped
  ad-hoc signed `.app`, seed-ит Background Shell sample, реально поднимает tray/dock
  adapters, проверяет accepted/rejected Background Shell action path и пишет JSON report
  c `startup-failed`, `no-permission`, `delivery-failed`, `action-rejected`. Native
  notification permission smoke включается явно через `--include-notifications`, чтобы
  обычный release smoke не зависел от macOS permission prompt.
...
- `./scripts/wails3-installed-app-smoke-macos.sh` добавлен как installed-app gate для
  уже установленного или явно переданного `Arlecchino.app`: report фиксирует bundle name,
  codesign verify, Gatekeeper status, process/resource snapshot, absence of real TCP
...
- Auto-update UX подключен в foreground stack: frontend читает runtime
  ...
  но background/startup policy остается quiet: foreground notification появляется
  только если найдено обновление или уже есть staged update (`available`/`staged`).
  `checking`, `not-available`, `manual-required` и `failed` не перебивают startup
  UX. Ручной `Check for Updates` доступен в Settings/Diagnostics и topbar `Actions`
  и всегда дает visible feedback: сначала `Checking for Updates`, затем result
  notification even if returned status is unchanged. Native notifications не дублируются.
...
- Private updater smoke прошел на local-alpha path `0.1.3-alpha.103`: установленный
  app нашел private GitHub release, скачал signed `arlecchino-macos-universal.zip`,
  прошел SHA256 + Ed25519 verify/stage, затем `Install and relaunch` заменил
  `/Applications/Arlecchino.app` и поднял новый BuildInfo. `0.1.1-alpha.101` и
  `0.1.2-alpha.102` считаются superseded: первый упирался в prerelease/latest
  resolution, второй - в AppleDouble entries внутри updater ZIP.
...
- Release draft flow зафиксирован в `docs/release-alpha-checklist.md`: one tag,
  ...
  checksum/manifest рядом, version только в tag/release metadata/manifest.
...
- Добавлен temp-only repro harness для pinned Wails `v3.0.0-alpha.87`
...
- Developer ID/notarized distribution будет добавлен soon, без зафиксированной
  даты; local-alpha auto-update теперь работает на собственном trust root: HTTPS
  + SHA256 + pinned Ed25519 public key. Private signing key остается вне repo.
...
| Protocol/file associations | Green | `arlecchino://` and file association payloads normalize through strict open-intent allowlist; real OS smoke now proves Wails handler entry and emitted dispatch through LaunchServices/AppleEvent routes. | Keep production default-handler claims out of this beta line; current builds are not Developer ID signed or notarized. |
...
- Developer ID/notarization is a soon-later gate with no committed date; keep
  trusted external macOS distribution claims out of beta copy until it lands.
...
24. Done: add native delivery live smoke. A production-shaped ad-hoc `.app` can now
    start tray/dock adapters, exercise accepted/rejected Background Shell actions and
    emit a JSON report with native failure states. Notification permission smoke is
    opt-in through `--include-notifications`.
25. Done: record Decision Gate Matrix. Lifecycle, bindings, promotion, detached windows,
    ...
    delivery.
26. Done: add packaged binary smoke matrix. The new macOS matrix script validates the
...
27. Done: harden detached Browser Preview return lifecycle. The return-to-main intent is
    now a pure helper contract that preserves preview identity, current URL payload,
    pinned state and return host/position.
28. Done: add production-shaped Wails v3 `.app` packaging and signing scripts. The bundle
...
29. Done: diagnose and recover real OS handoff. The Arlecchino harness now records Wails
...
37. Done: add installed-app smoke gate. `wails3-installed-app-smoke-macos.sh` validates
...
39. Done: replace in-app notification UX. The old single-toast and small save-status
...
46. Done: record updater evidence and quiet startup policy. Private updater smoke passed
    for `0.1.3-alpha.103` via signed ZIP -> verify/stage -> user-confirmed
    install/relaunch, with `0.1.1`/`0.1.2` marked superseded. Startup/background
    checks now stay silent for `checking`, `not-available`, `manual-required` and
    `failed`, only surfacing foreground notifications when an update exists or is
    staged. Long backend diagnostics remain in updater status/report while notification
    cards show short user-facing messages. `wails3-private-updater-live-smoke-macos.sh`
    records release/app ZIP evidence without committing artifacts.
47. Done: compact update release notes and action feedback. Updater foreground cards now
    summarize curated release notes, hide raw commit digests, expose longer notes behind
    `Details`, show a bottom progress line for check/download/install states, and guard
    download/apply actions against repeated clicks while an updater operation is running.
    Release manifest generation rejects notes that look like raw git log output.
48. Done: make updater version checks build-aware. Update manifests now carry `build`
    alongside `version`, runtime comparison uses `version` first and `build` second, and
    foreground cards can show target build labels. The `0.1.5-alpha.106` release is the
    compatibility bridge because installed builds before this fix only compare `version`.
